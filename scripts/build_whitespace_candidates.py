...
def main():
 ...
 for area in c['study_scope']['areas']:
  ...
  for cell in h3.polygon_to_cells(poly,c['candidate_grid']['resolution']):
   ...
   if c['candidate_distance_km']['minimum_from_active_branch']<=nearest<=c['candidate_distance_km']['maximum_from_active_branch']:
    rows.append({'cell_id':cell,'study_area_id':area['id'],'latitude':round(lat,6),'longitude':round(lon,6),'nearest_active_branch_distance_km':round(nearest,6),'label':'RESEARCH_REQUIRED','confidence':0,'limitations':['No source-backed built-environment or demand input; no GROW/WATCH/SKIP label.']})
 ...
 for row in rows:
  ...
  row['limitations']=['User-validated urban-context anchor proximity only; not demand evidence or an opening recommendation.']
