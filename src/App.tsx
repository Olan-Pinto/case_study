      ...
      <div className="layout">
        ...
        <section className="map-panel" id="network-map" aria-label="Network map and layer controls">
          ...
          <div className="whitespace-legend" aria-label="Whitespace research-cell legend">
            ...
            <span><i className="legend-deprioritize" />Deprioritize</span>
            <span><i className="legend-required" />Insufficient evidence</span>
          </div>
...
function WhitespaceDetail({ candidate, residentialContext, priority }: { candidate: any, residentialContext: any, priority: any }) {
  const residentialAvailable = candidate.residential_context_coverage_status === 'available'
  const labels: Record<string, string> = { PRIORITIZE_RESEARCH: 'Prioritize research', WATCH_RESEARCH: 'Watch', DEPRIORITIZE_RESEARCH: 'Deprioritize', RESEARCH_REQUIRED: 'Insufficient evidence' }
  const factorNames: Record<string, string> = { residential_intensity: 'Residential intensity', own_network_spacing: 'Own-network spacing', urban_context_proximity: 'Urban-context proximity', competitor_market_validation: 'Competitor market validation' }
  return <><p className="eyebrow">Whitespace evidence</p><h2>{labels[candidate.label]}</h2><p className="place">{candidate.study_area_id.replaceAll('_', ' ')}</p><section className="priority-summary"><h3>Research-priority score</h3>{candidate.research_priority_score === null ? <><strong className="priority-score">No score</strong><p>Required residential evidence is unavailable, so this cell is not ranked.</p></> : <><strong className="priority-score">{candidate.research_priority_score.toFixed(2)} / 100</strong><p>A higher score moves this cell earlier in the manual research queue. It is not a chance of success or an opening recommendation.</p></>}</section><dl className="facts"><div><dt>Nearest active branch</dt><dd>{candidate.nearest_active_branch_distance_km.toFixed(2)} km</dd></div><div><dt>Nearest high-priority anchor</dt><dd>{candidate.nearest_high_priority_anchor_km.toFixed(2)} km</dd></div><div><dt>Limited competitor pressure</dt><dd>{candidate.competitor_pressure_lower_bound.toFixed(2)}</dd></div><div><dt>Evidence confidence</dt><dd>{candidate.confidence}%</dd></div></dl>{candidate.factor_contributions && <section className="score-breakdown"><h3>Why it received this score</h3><ul>{Object.entries(candidate.factor_contributions).map(([name, contribution]) => <li key={name}><span>{factorNames[name]}</span><small>{(candidate.factor_values[name] * 100).toFixed(0)} / 100 × {Math.round(priority.weights[name] * 100)} points</small><strong>+{Number(contribution).toFixed(2)}</strong></li>)}</ul><p className="score-equation">Total: {Object.values(candidate.factor_contributions).map((value) => Number(value).toFixed(2)).join(' + ')} = <strong>{candidate.research_priority_score.toFixed(2)}</strong></p></section>}<details className="confidence-details"><summary>How was {candidate.confidence}% confidence calculated?</summary>{candidate.confidence ? <><ul>{Object.entries(priority.confidence.components).map(([name, value]) => <li key={name}><span>{name.replaceAll('_', ' ')}</span><strong>{Math.round(Number(value) * 100)}%</strong></li>)}</ul><p>{priority.confidence.method}</p></> : <p>Confidence is withheld with the score because required residential evidence is missing.</p>}</details><section className="residential-context"><h3>Residential context · 2025 estimate</h3>{residentialAvailable ? <><p><strong>{Math.round(candidate.estimated_residents_2025).toLocaleString()}</strong> modelled residents in this hexagon</p><p>Relative residential intensity: <strong>{candidate.residential_intensity_percentile_within_study_area.toFixed(0)} / 100</strong> within this study area</p></> : <p><strong>Unavailable.</strong> No valid raster pixels were present, so the value remains missing—not zero.</p>}<p>{residentialContext.interpretation}</p><p>{residentialContext.label_role}</p><a href={residentialContext.source_url} target="_blank" rel="noreferrer">Open WorldPop source ↗</a></section><section className="caution"><h3>Important limitation</h3><p>{candidate.limitations[0]}</p></section></>
}
