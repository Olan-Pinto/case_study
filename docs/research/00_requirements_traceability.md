...
| Scenario sensitivity | Existing branch-health evidence and bounded named weights | Deterministic replay of the same scorecard | Scenario selector and branch delta panel | Baseline, delta, changed label, weights, fixed-evidence limitation | Baseline replay, weight-bound, label-change tests | Change a strategic priority and inspect what actually moved |
